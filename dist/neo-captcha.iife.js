...
`;const A=/Mobi|Android|iPhone|iPad|iPod/i.test(navigator.userAgent),b=document.getElementById("startOverlay"),ie=document.getElementById("howTo"),z=document.getElementById("submit"),D=document.getElementById("start"),c=document.getElementById("captchaCanvas"),e=c.getContext("2d"),v=document.getElementById("timeCanvas"),p=v.getContext("2d");if(!e||!p)throw new Error("Canvas context could not be initialized.");const L=6e3;let h=[255,0,0],B=!1,m=[],d=0,P=0,$=0,f=0,g=!1,j="",R=0,M=0,C,E=oe;const ce=document.getElementById("howToCaption"),J=document.getElementById("howToText"),Y=document.getElementById("howToIcon");J.style.display=E?"block":"none",Y.innerText=E?"expand_less":"expand_more",ce.addEventListener("click",()=>{E=!E,J.style.display=E?"block":"none",Y.innerText=E?"expand_less":"expand_more"}),D.addEventListener("click",X);const se=document.getElementById("signalIcon");se.innerText=A?"vibration":"hearing",ae||(ie.style.display="none");function le(){console.log("userAgent: "+navigator.userAgent),console.log('"vibrate" in navigator: '+("vibrate"in navigator)),A&&"vibrate"in navigator?(navigator.vibrate(200),f>0?m.push({action:"react",time:f-Date.now()}):f=Date.now()):he()}async function X(){const o=document.getElementById("wrapper");o.style.display="block",D.style.display="none";const n=C?{id:C.toString()}:{},t=await(await fetch("https://neo-captcha-backend.fly.dev/api/generate-captcha",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)})).json();if(console.log(t),t.img){const i=document.getElementById("bg");i.style.display="inline-block",b.style.display="inline-block",j=`data:image/png;base64,${t.img}`,R=t.pointSize,M=t.thumbSize,h=t.color,C=BigInt(t.id);const w=document.getElementById("container");if(w.style.height="320px",c.height=c.width,!e||!p)throw new Error("Canvas context could not be initialized.");e.fillStyle="rgba(0, 0, 0)",e.fillRect(0,0,c.width,c.height),p.fillStyle=`rgba(${h[0]}, ${h[1]}, ${h[2]})`,$=Date.now(),setTimeout(()=>le(),t.suspense)}}function re(){d=Date.now(),q()}function q(){if(!p)throw new Error("Canvas context could not be initialized.");const o=Date.now()-d,n=Math.max(L-o,0),a=n/L*v.width;p.clearRect(0,0,v.width,v.height),p.fillStyle=`rgba(${h[0]}, ${h[1]}, ${h[2]})`,p.fillRect(0,0,a,v.height),n>0&&g?requestAnimationFrame(q):n<=0&&g?(console.log("Time's up!"),P=d+L,Q()):(p.fillStyle="rgba(255, 255, 255, 0.8)",p.fillRect(0,0,v.width,v.height))}function G(o,n){if(!e)throw new Error("Canvas context could not be initialized.");e.clearRect(0,0,c.width,c.height),e.beginPath(),e.arc(o-1,n-1,M/2,0,Math.PI*2),e.fillStyle=`rgba(${h[0]}, ${h[1]}, ${h[2]}, 0.2)`,e.fill()}function U(){d==0&&(f>0?m.push({action:"react",time:Date.now()-f}):f=Date.now()),A&&"vibrate"in navigator&&navigator.vibrate(200)}b.addEventListener("mousedown",U),b.addEventListener("touchstart",U);function O(o,n){let a,t;return o instanceof MouseEvent?(a=o.clientX-n.left,t=o.clientY-n.top):(a=o.touches[0].clientX-n.left,t=o.touches[0].clientY-n.top),{x:a,y:t}}function V(o){const n=c.getBoundingClientRect();let{x:a,y:t}=O(o,n);d>0&&m.push({action:"down",enabled:g,x:a,y:t,time:Date.now()-d}),g&&(B=!0,G(a,t))}c.addEventListener("mousedown",V),c.addEventListener("touchstart",V);function K(o){const n=c.getBoundingClientRect();let{x:a,y:t}=O(o,n);d>0&&m.push({action:"move",enabled:g,drawing:B,x:a,y:t,time:Date.now()-d}),g&&B&&G(a,t)}c.addEventListener("mousemove",K),c.addEventListener("touchmove",K);function _(){if(d==0){m.push({action:"start",time:Date.now()-$}),g=!0,z.disabled=!1;const o=document.getElementById("bg");if(o.src=j,re(),!e)throw new Error("Canvas context could not be initialized.");e.clearRect(0,0,c.width,c.height),b.style.display="none"}}b.addEventListener("mouseup",_),b.addEventListener("touchend",_),b.addEventListener("touchcancel",_);function H(o){const n=c.getBoundingClientRect();let{x:a,y:t}=O(o,n);if(d>0&&m.push({action:"up",enabled:g,x:a,y:t,time:Date.now()-d}),d>=0&&g){if(B=!1,m.push({action:"point",x:a,y:t,time:Date.now()-d}),!e)throw new Error("Canvas context could not be initialized.");e.clearRect(0,0,c.width,c.height),e.beginPath(),e.arc(a,t,R/2,0,Math.PI*2),e.fillStyle=`rgba(${h[0]}, ${h[1]}, ${h[2]})`,e.fill()}}c.addEventListener("mouseup",H),c.addEventListener("touchend",H),c.addEventListener("touchcancel",H),z==null||z.addEventListener("click",Q);async function Q(){if(!g)return;if(g=!1,z.disabled=!0,!e||!p)throw new Error("Canvas context could not be initialized.");e.fillStyle="rgba(255, 255, 255, 0.8)",e.fillRect(0,0,c.width,c.height),p.fillStyle="rgba(255, 255, 255, 0.8)",p.fillRect(0,0,v.width,v.height),P===0&&(P=Date.now());const o=P-d;m.push({action:"end",time:o});const n={id:C==null?void 0:C.toString(),activity:m},t=await(await fetch("https://neo-captcha-backend.fly.dev/api/validate-captcha",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)})).json();e.lineJoin="round",e.lineCap="round";const i=c.width*.1;let w=c.width/2,I=c.height/2;const s=c.width/3;if(t.valid){e.strokeStyle="rgba(0, 0, 0, 0.02)",e.lineWidth=i+20;let l=w+i/8,r=I+i/4;x(s,l,r),e.lineWidth=i+17,x(s,l,r),e.lineWidth=i+14,x(s,l,r),e.lineWidth=i+11,x(s,l,r),e.lineWidth=i+8,x(s,l,r),e.strokeStyle="rgba(0, 160, 0)",e.lineWidth=i,x(s,w,I)}else if(t.retry){e.strokeStyle="rgba(0, 0, 0, 0.02)",e.lineWidth=i+20;let l=w+i/8,r=I+i/4;S(s,l,r),e.lineWidth=i+17,S(s,l,r),e.lineWidth=i+14,S(s,l,r),e.lineWidth=i+11,S(s,l,r),e.lineWidth=i+8,S(s,l,r),e.strokeStyle="rgba(0, 80, 255)",e.lineWidth=i,S(s,w,I)}else{e.strokeStyle="rgba(0, 0, 0, 0.02)",e.lineWidth=i+20;let l=w+i/8,r=I+i/4;k(s,l,r),e.lineWidth=i+17,k(s,l,r),e.lineWidth=i+14,k(s,l,r),e.lineWidth=i+11,k(s,l,r),e.lineWidth=i+8,k(s,l,r),e.strokeStyle="rgba(255, 0, 0)",e.lineWidth=i,k(s,w,I)}t.valid&&T&&T.onSuccess?T.onSuccess():t.retry?setTimeout(()=>{de(),X()},500):T&&T.onFailure&&T.onFailure()}function x(o,n,a){if(!e)throw new Error("Canvas context could not be initialized.");const t=o/2;e.beginPath(),e.moveTo(n-t/2,a+t),e.lineTo(n-t-t/2,a),e.moveTo(n-t/2,a+t),e.lineTo(n+o-t/2,a-t),e.stroke()}function k(o,n,a){if(!e)throw new Error("Canvas context could not be initialized.");const t=o/2;e.beginPath(),e.moveTo(n-t,a-t),e.lineTo(n+t,a+t),e.moveTo(n+t,a-t),e.lineTo(n-t,a+t),e.stroke()}function S(o,n,a){if(!e)throw new Error("Canvas context could not be initialized.");const t=o/2;e.beginPath(),e.moveTo(n-t,a),e.lineTo(n-t+1,a),e.moveTo(n,a),e.lineTo(n+1,a),e.moveTo(n+t,a),e.lineTo(n+t+1,a),e.stroke()}function de(){B=!1,m=[],d=0,P=0,$=0,f=0,g=!1,j="",R=0,M=0;const o=document.getElementById("wrapper");o.style.display="none",D.style.display="block"}const y=new AudioContext,he=()=>{y.state==="suspended"?y.resume().then(()=>Z()):Z()},Z=()=>{N(285,.12),N(852,.12,.12),N(528,.12,.24),f>0?m.push({action:"react",time:f-Date.now()}):f=Date.now()};function N(o,n,a=0){let t=y.createOscillator(),i=y.createGain();t.type="sine",t.frequency.value=o,i.gain.value=.1,t.connect(i),i.connect(y.destination),t.start(y.currentTime+a),t.stop(y.currentTime+a+n)}}return window.NeoCAPTCHA={render:F},W.renderCaptcha=F,Object.defineProperty(W,Symbol.toStringTag,{value:"Module"}),W}({});
